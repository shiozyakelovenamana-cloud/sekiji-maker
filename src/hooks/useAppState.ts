...
export function useAppState() {
  ...
  const setLayout    = useCallback((layout: LayoutConfig) => setState(prev => {
    // 接待モードでレイアウト変更時、人数が新レイアウトの席数を超える場合はデフォルト人数にリセット
    if (prev.mode === 'hospitality') {
      const seatCount = (() => {
        switch (layout.type) {
          ...
          case 'counter':  return layout.config.seatCount + 1; // 客席+大将正面
          case 'taxi': case 'elevator': return 4;
          default: return 99;
        }
      })();
      const total = Object.values(prev.counts).reduce((s, v) => s + (v as number), 0);
